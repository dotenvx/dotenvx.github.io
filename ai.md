    ...
    <div class="armor-shell">
      {% capture about_essay %}
        <div class="design-prose">
          ...
          <p>Help your company advance this work through {% include components/design-link.html href="/corporate" label="corporate involvement" %}.</p>
        </div>
      {% endcapture %}
...
(() => {
  ...
  hit.addEventListener('keyup', event => {
    if (event.key !== ' ' && event.key !== 'Enter') return;
    event.preventDefault();
    if (key.classList.contains('is-pressed')) changeTheme();
    release();
  });
